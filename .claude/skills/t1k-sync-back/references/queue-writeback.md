...
```
{projectRoot}/.claude/telemetry/pending-skill-updates.jsonl
```
...
The processor's circuit breaker increments `failures` on this entry. After 5 consecutive failures the entry is marked `permanently_failed` and dropped from the queue. A `[t1k:lesson-stale]` marker is emitted so the maintainer is aware.
