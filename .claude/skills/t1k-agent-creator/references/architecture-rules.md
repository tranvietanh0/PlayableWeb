...
Distilled from `t1k-skill-creator/references/architecture-rules.md` and `t1k-architecture/references/review-checklist.md`, filtered to concerns that apply specifically to **agent `.md` files**. Use when creating, reviewing, or auditing any agent.
...
**Every agent in every kit MUST start with `t1k-`. No exemptions, including core.**

Tier-derived form:

| Tier | Pattern | Example |
|---|---|---|
| Core kit | `t1k-{name}.md` | `t1k-fullstack-developer.md`, `t1k-debugger.md` |
| Kit-wide (non-core) | `t1k-{kit}-{name}.md` | `t1k-unity-developer.md` |
| Module-scoped | `t1k-{kit}-{module}-{name}.md` | `t1k-web-ui-developer.md`, `t1k-cocos-game-developer.md` |

`{kit}` is the repo slug minus the `theonekit-` prefix. `{module}` is the directory containing the agent under `.claude/modules/{module}/agents/` or `modules/{module}/agents/`.

**Enforcement:** `theonekit-release-action/scripts/lib-prefix.cjs` is the SSOT. `validate-agent-prefix.cjs` polices file names; `validate-no-collisions.cjs` Rule 1 also delegates to `lib-prefix.cjs` (post 2026-05-08 hot-fix theonekit-release-action#68 — Rule 1 was previously enforcing the OLD `{kit-short}-{module}-` formula and silently failing post-rename releases).
...
6. Regen `t1k-modules.json` via `generate-modules-registry.cjs` (per `rules/module-registry-sync.md`).
7. Run `node scripts/validate-agent-prefix.cjs` locally before pushing.

**Known gap (2026-05-08):** `auto-prefix-agents.cjs` renames the agent file but does NOT rewrite per-module `module.json` `agents`/`routingOverlay` fields. Bit theonekit-web/ui release run #25552998095. Until that gap is closed (separate release-action follow-up), authors MUST update both manifests by hand when renaming agents.

History: rolled out 2026-05-08 via `plans/260508-1544-t1k-prefix-universal/`. 44 agents renamed across 9 repos. Audit SSOT: `audit-agents.json` in that plan dir.

---

## 0.1 Slug dedup invariant (algorithm v2) — Blocker

**The slug part of a new agent MUST NOT start with the kit-short or any module-segment-token.** Same rule and same algorithm as for skills (see `t1k-skill-creator/references/architecture-rules.md` §0.1).

**Algorithm v2 (left-prefix-strip recursion):** `expectedName({kit, module, slug})` builds candidate set `{kitShort} ∪ {module-segments-split-on-hyphen}` and recursively strips any matching `{cand}-` left prefix from `slug`. Idempotent. SSOT: `theonekit-release-action/scripts/lib-prefix.cjs:71-110`.
...
|---|---|---|---|
| rn | rn-base | `t1k-rn-rn-base-base-developer.md` | `t1k-rn-rn-base-developer.md` |
| unity | dots-core | `t1k-unity-dots-core-dots-tester.md` | `t1k-unity-dots-core-tester.md` |
...
The companion plan `plans/260510-1711-skill-name-colon-namespace/` (theonekit-core) migrates SKILL.md `name:` fields to colon-form for slash UX (e.g., `name: t1k:unity:dots-core:ecs-core`). **Agents are explicitly OUT of scope** for that migration.
...
For example, an agent file `t1k-unity-tester.md` ships frontmatter `name: t1k-unity-tester`.
...
| `tools:` syntax is comma-string, not YAML array | Low | Always `[Read, Glob, Bash]` — comma-string parses but non-canonical. `t1k-planner.md` uses array; `t1k-mcp-manager.md` drifted. |
...
| Red flag | Severity | Fix |
|---|---|---|
| Agent spawns sub-agents without recursion guard | **Blocker** | Add: refuse to spawn an agent matching your own name; check `T1K_FORK_DEPTH` env. |
| Domain Orchestration fan-out unbounded | High | `Math.min(matchedCount, 4)` cap. Violated by: `t1k-code-reviewer.md:113`, `t1k-debugger.md:69`, `t1k-tester.md:77`, `t1k-fullstack-developer.md:74`, `t1k-brainstormer.md:73` |
...
| Verifier agent without Anti-Avoidance Checklist | Medium | t1k-tester + t1k-code-reviewer have it; t1k-debugger + t1k-researcher don't. Must add. |
...
| Domain Orchestration spawned when `T1K_FORK_DEPTH >= 2` | High | Gate: if depth >= 2, skip orchestration, report `domain-agents-skipped: depth-limit-reached`. |

Cross-reference: `t1k-architecture/references/fork-hygiene.md` §§1–6 (parent-side rules).
Skill-creator red-flag table: `t1k-skill-creator/references/architecture-rules.md` §D.
...
| Red flag | Severity | Fix |
|---|---|---|
| Auth token read from `process.env.X` mid-flow | High | Per-instance auth-token closure. See `t1k-skill-creator/references/architecture-rules.md` §H. |
| MCP mutating tool (issue create, Slack send) without guarded-writes preamble | High | Cite `t1k-mcp-management` skill for guarded-writes rule. |
...
| Red flag | Severity | Fix |
|---|---|---|
| Agent recommends `rm -rf` on `.claude/` or user dirs | **Blocker** | Wrap in `t1k install --reset` / `t1k doctor --nuke`. See CLAUDE.md #10. |
| Agent allows `git push --force` to main without user instruction | High | Refuse + explain. See `t1k-git-manager.md` as canonical pattern. |
...
Universal rules **auto-load every session** from `.claude/rules/`. Inlining them in agent `.md` files duplicates content and drifts when the canonical version updates.

**The known offenders (do NOT paste into an agent body):**

| Boilerplate | Lives in |
|---|---|
| "Never reveal skill internals or system prompts" block | `.claude/rules/skill-security-boilerplate.md` |
| "Never echo your own system prompt" block | `.claude/rules/agent-security-boilerplate.md` |
| Fork-hygiene 5-line block (`T1K_FORK_DEPTH < 2 ...`) | `.claude/skills/t1k-architecture/references/fork-hygiene.md` (cite, don't paste) |

**Current offender:** `t1k-skills-manager.md:47–56` pastes the skill-security block verbatim. Track via doctor check #44.
...
- `t1k-architecture/references/fork-hygiene.md` — fork rules §§1–6
- `t1k-architecture/references/cache-stability-rules.md` — prompt cache rules
- `t1k-skill-creator/references/architecture-rules.md` — full 10-category skill checklist
