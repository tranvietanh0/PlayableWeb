...
Pre-merge checklist for any new or updated agent `.md` file. Mirrors `t1k-skill-creator/references/validation-checklist.md` scoped to agents.
...
- [ ] Recursion guard documented — refuse to spawn an agent matching own name; check `T1K_FORK_DEPTH` env
- [ ] Fan-out cap declared — `Math.min(N, 4)` or explicit constant; no "for each → spawn" without cap
- [ ] Domain Orchestration gated: `IF T1K_FORK_DEPTH >= 2: skip, report "domain-agents-skipped: depth-limit-reached"`
- [ ] Sub-agent `tools:` declared in child frontmatter (not invented at spawn site)

## Verifier-class agents (t1k-tester, reviewer, t1k-debugger, doctor, qa-*)
...
- [ ] Body does NOT paste fork-hygiene block verbatim — cites `t1k-architecture/references/fork-hygiene.md` instead
- [ ] **Caught by:** doctor check #44 (now extended to `agents/*.md`)

## Destructive operations (agents with `Bash`/`git`/destructive tools)

- [ ] Body includes backup-before-destruction clause (references CLAUDE.md #10)
- [ ] No raw `rm -rf .claude/` — must wrap in `t1k install --reset` or `t1k doctor --nuke`
...
- [ ] If agent has routing role: added to `t1k-routing-{layer}.json`
- [ ] Agent name and purpose documented in team/registry awareness
- [ ] Run `/t1k:doctor` after adding to verify no role collision and no file overlap
...
**Architecture depth:** `references/architecture-rules.md` (this skill) + `t1k-skill-creator/references/architecture-rules.md` §D (skill-side fork rules)
