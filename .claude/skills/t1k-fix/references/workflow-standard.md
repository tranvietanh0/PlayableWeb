...
2. Activate `ck:debug` skill. Use `t1k-debugger` subagent if needed.
...
Use `t1k-code-reviewer` subagent.
...
- Update docs if needed via `t1k-docs-manager`
- Ask to commit via `t1k-git-manager` subagent
...
| 2 | `ck:debug`, `ck:sequential-thinking`, `t1k-debugger` subagent, parallel `Explore`, (`ck:problem-solving` auto) |
| 3 | `ck:problem-solving` (if stuck), `ck:sequential-thinking` (complex logic) |
| 4 | `t1k-tester` subagent, parallel `Bash` verification |
| 5 | `t1k-code-reviewer` subagent |
| 6 | `ck:project-management`, `t1k-git-manager`, `t1k-docs-manager` subagents |
