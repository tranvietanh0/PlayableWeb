...
Use `t1k-researcher` subagent for external knowledge.
...
Use `t1k-planner` subagent to create implementation plan.
...
- Use `t1k-docs-manager` subagent for documentation
- Use `t1k-git-manager` subagent for commit
...
| 3 | `t1k-researcher` (runs parallel with steps 1+2) |
| 4 | `ck:brainstorm` |
| 5 | `t1k-planner` |
| 6 | `ck:problem-solving`, `ck:sequential-thinking`, `ck:context-engineering` |
| 7 | `t1k-tester`, parallel `Bash` verification |
| 8 | `t1k-code-reviewer` |
| 9 | `ck:project-management`, `t1k-docs-manager`, `t1k-git-manager` |
