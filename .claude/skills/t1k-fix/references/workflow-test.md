...
Use `t1k-tester` agent. Fix all syntax errors before running tests.
...
Use `t1k-debugger` agent for root cause analysis.
...
Use `t1k-planner` agent for fix strategy.
...
Use `t1k-tester` agent. If tests still fail → keep T5 `in_progress`, loop back to Step 2.

`TaskUpdate(T5, status="completed")`

### Step 6: Review
`TaskUpdate(T6, status="in_progress")`
Use `t1k-code-reviewer` agent.
