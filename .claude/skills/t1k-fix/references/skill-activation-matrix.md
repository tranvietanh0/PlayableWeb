...
|----------|---------------|
| `t1k-debugger` | Root cause unclear, need deep investigation (Step 2) |
...
| `t1k-researcher` | External docs needed, latest best practices (Deep only) |
| `t1k-planner` | Complex fix needs breakdown, multiple phases (Deep only) |
| `t1k-tester` | After implementation, verify fix works (Step 5) |
| `ck:code-review` | After fix, verify quality and security (Step 5) |
| `t1k-git-manager` | After approval, commit changes (Step 6) |
| `t1k-docs-manager` | API/behavior changes need doc updates (Step 6) |
| `t1k-project-manager` | Major fix impacts roadmap/plan status (Step 6) |
| `t1k-fullstack-developer` | Parallel independent issues (each gets own agent) |
...
| 2+ independent issues | Task trees + `t1k-fullstack-developer` agents per issue |
...
| Standard | Above + Tasks, `ck:problem-solving` (auto), `ck:project-management`, `t1k-tester`, parallel `Explore` |
| Deep | All above + `ck:brainstorm`, `ck:context-engineering`, `t1k-researcher`, `t1k-planner` |
| Parallel | Per-issue Task trees + `ck:project-management` + `t1k-fullstack-developer` agents + coordination via `TaskList` |
...
| Step 6: Finalize | Report → `t1k-docs-manager` → `TaskUpdate` → `t1k-git-manager` → run `/t1k:watzup` or record session summary |
...
| "latest docs", "best practice" | `t1k-researcher` subagent |
