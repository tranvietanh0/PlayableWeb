...
- Typical workflow: review with `contract-review` → edit the Markdown → `t1k-contract beautify` to produce the final `.docx` / `.pdf`.
...
9. **Render and send.**
   - Hand off to `t1k-contract beautify` to produce `.docx` / `.pdf`.
...
- **Pure rendering/formatting work** — use `t1k-contract` instead.
