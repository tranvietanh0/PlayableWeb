...
```js
/**
 * Scans `.claude/modules/*/module.json` and reports ...
 */
```
...
```js
/**
 * Scans `.claude/modules/<name>/module.json` and reports ...
 */
```

CI defence: `validate-post-inject-syntax.cjs` runs `node --check` on every `.cjs`/`.js`/`.mjs` under `.claude/hooks/` and `.claude/skills/` — it will fail PRs that reintroduce this pattern. Local check before commit:

```bash
node -c path/to/your-script.cjs
```

**Bug trail:** first shipped to `main` in `check-module-detect-coverage.cjs` (2026-04-18), fixed in theonekit-core#31. CI gate extended in theonekit-release-action#6.
