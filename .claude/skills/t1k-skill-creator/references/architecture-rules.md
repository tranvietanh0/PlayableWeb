...
Distilled from `t1k-architecture/references/review-checklist.md` (Claude Code reverse-engineering canon) and applied to TheOneKit skill design. Use this when creating, reviewing, or auditing any skill — these rules catch failure modes that Skillmark formatting checks (frontmatter, line cap, gotchas) cannot.
...
## 0. Universal `t1k-` prefix (MANDATORY) — Blocker

**Every skill in every kit MUST start with `t1k-`. No exemptions, including core.**

Tier-derived form:

| Tier | Pattern | Example |
|---|---|---|
| Core kit | `t1k-{name}` | `t1k-doctor`, `t1k-cook` |
| Kit-wide (non-core) | `t1k-{kit}-{name}` | `t1k-unity-mcp-skill` |
| Module-scoped | `t1k-{kit}-{module}-{name}` | `t1k-unity-dots-ecs-core`, `t1k-web-ui-styling` |

`{kit}` is the repo slug minus the `theonekit-` prefix. `{module}` is the directory under `.claude/modules/{module}/skills/` or `modules/{module}/skills/`.

**Enforcement:** `theonekit-release-action/scripts/lib-prefix.cjs` is the SSOT (functions: `expectedName`, `expectedPrefix`, `conforms`, `warnOnly`). CI runs three strict gates on every push:
...
Default since 2026-05-08 is strict (`T1K_GATE_STRICT=1`); `T1K_GATE_WARN_ONLY=1` is the explicit downgrade for emergency rollouts.
...
5. Regen `t1k-modules.json` via `generate-modules-registry.cjs` (per `rules/module-registry-sync.md`).
6. Run `node scripts/validate-skill-prefix.cjs` locally before pushing.

History: rolled out 2026-05-08 via `plans/260508-1544-t1k-prefix-universal/`. 326 skills + 44 agents renamed across 9 repos in one session. Audit SSOT: `audit-skills.json` in that plan dir.

---

## 0.1 Slug dedup invariant (algorithm v2) — Blocker

**The slug part of a new skill MUST NOT start with the kit-short or any module-segment-token.** The prefixer would strip them at release time anyway, so authoring redundant prefixes only creates churn (and shipped the double-prefix names — `t1k-rn-rn-base-base-architecture` etc. — that the 2026-05-10 dedup migration cleaned up).

**Algorithm v2 (left-prefix-strip recursion):** `expectedName({kit, module, slug})` builds a candidate set `{kitShort} ∪ {module-segments-split-on-hyphen}` and recursively strips any matching `{cand}-` left prefix from `slug` until no further match. Idempotent. See `theonekit-release-action/scripts/lib-prefix.cjs:71-110` for the canonical implementation.

**Examples (do NOT do these):**

| Kit | Module | BAD slug | What ships (after v2 strip) |
|---|---|---|---|
| rn | rn-base | `base-architecture` | `t1k-rn-rn-base-architecture` (the leading `base-` is stripped) |
| cocos | cocos-animation | `cocos-playable-juice` | `t1k-cocos-cocos-animation-playable-juice` (leading `cocos-` stripped) |
| designer | design-base | `design-data-authoring` | `t1k-designer-design-base-data-authoring` (leading `design-` stripped) |
| unity | dots-core | `dots-architecture` | `t1k-unity-dots-core-architecture` (leading `dots-` stripped) |

**Examples (CORRECT):**

| Kit | Module | GOOD slug | Ships as |
|---|---|---|---|
| rn | rn-base | `architecture` | `t1k-rn-rn-base-architecture` |
| unity | dots-core | `ecs-core` | `t1k-unity-dots-core-ecs-core` |
| designer | design-puzzle | `game-design` | `t1k-designer-design-puzzle-game-design` |

**Note on shorthand prefixes (e.g., `perf-` for module `rn-performance`):** Algorithm v2 leaves these alone — `perf` is not a literal repeat of `performance`. Per-name human disposition required if you want to dedup shorthand. As of 2026-05-10, the four `t1k-rn-rn-performance-perf-*` skills ship with the `perf-` prefix intact (algorithm v2 default).
...
**When you add a skill, register its keyword mappings in the owning module's `t1k-activation-*.json` fragment using the BARE-slug form, not the full-prefixed form.**

The CI prefixer's `auto-prefix-skills.cjs::buildSelfHealMap()` accepts five plausible bare forms per real skill dir:

| Form | Example (real dir: `t1k-rn-rn-base-architecture`) |
|---|---|
| Identity (full-prefixed) | `t1k-rn-rn-base-architecture` |
| Kit + module strip | `architecture` |
| Kit-only strip | `rn-base-architecture` |
| Module-only strip | `rn-rn-base-architecture` |
| `t1k-` only strip | `rn-rn-base-architecture` |
...
NOT: `["t1k-rn-rn-base-navigation"]` (full-prefixed — works but verbose) or `["rn-navigation"]` (legacy `{kit}-{slug}` form skipping the module segment — NOT in the self-heal accept set, will fail the per-PR drift safety-net gate `validate-activation-skill-resolution.cjs`).
...
**Skill rename + activation update:** if you rename a skill directory, also update every `t1k-activation-*.json` `skills[]` array that references the old slug (per `t1k-sync-back/SKILL.md` operational notes). The prefixer self-heals on the next CI run, but shipping the activation update in the same PR keeps the SSOT consistent.

---

## 0.2 Slash UX `name:` field colon-form (PENDING — colon-namespace migration)

**Status:** PLANNED rollout per `plans/260510-1711-skill-name-colon-namespace/` (theonekit-core). Phase 0–2 (research + release-action helpers + this docs update) shipped 2026-05-10 during the dedup soak window. Phase 3 (consumer-kit smoke) and Phase 4 (kit fan-out) are deferred until dedup soak ends 2026-05-17 — see `plans/260510-1711-skill-name-dedup/SEQUENCING-WITH-COLON-PLAN.md` Hard rule 2.

**Target invariant:** the SKILL.md frontmatter `name:` field uses **colon** as the inter-tier separator (`t1k:kit:module:slug`), while the directory name keeps **hyphens** (`t1k-kit-module-slug/`). Two separators, two SSOTs:

| Tier | Directory (filesystem) | `name:` field (slash UX) | Example |
|---|---|---|---|
| Core | `t1k-{slug}/` | `t1k:{slug}` | `t1k-cook/` → `name: t1k:cook` *(already shipped)* |
| Kit-wide non-core | `t1k-{kit}-{slug}/` | `t1k:{kit}:{slug}` *(2-tier — Q1 resolved 2026-05-10)* | `t1k-unity-mobile/` → `name: t1k:unity:mobile` |
| Module-scoped | `t1k-{kit}-{module}-{slug}/` | `t1k:{kit}:{module}:{slug}` | `t1k-unity-dots-core-ecs-core/` → `name: t1k:unity:dots-core:ecs-core` |
...
- New skills in **theonekit-core** SHOULD use colon-form `name:` (the convention is already shipped for core — every new core skill should match).
...
**Why this isn't enforced yet:** transitional alias support is not available in the Claude Code harness (`plans/260510-1711-skill-name-colon-namespace/harness-research.md`). Hard cutover at Phase 4 is the only path. Premature enforcement would break consumer kit installs mid-soak.
...
**Correct form for every agent** — see `t1k-agent-creator/references/architecture-rules.md` §0.2 for the canonical example. (Code block omitted here to keep the example out of the cross-refs gate's prose-scan target — agent file paths inside slash-prefixed comments would be flagged as broken skill refs.)
...
| Red flag | Severity | Fix |
|---|---|---|
| Live shell substitution in skill body (bang-prefixed backtick command form) | High | Move live data to a tool call AFTER the cached prefix; keep body static |
...
| Dynamic agent list in tool description | Medium | Move to attachment message (Claude Code saved 10.2% `cache_creation` tokens this way) |
...
## I. Destructive operation safety (CLAUDE.md #10)

| Red flag | Severity | Fix |
|---|---|---|
| Skill recommends raw `rm -rf` on user files/dirs | Blocker | Wrap in CLI command (`t1k install --reset`, `t1k doctor --nuke`) that backs up to `~/.claude-backup-{ISO-ts}/` first |
...
## J. AI-driven design (per CLAUDE.md principle #8)

| Red flag | Severity | Fix |
|---|---|---|
| Skill puts policy/decision logic in pure-CLI scripts when AI could add semantic context | Medium | CLI/scripts emit machine-parseable facts (JSON + file excerpts); skill uses Claude to decide policy and explain rationale |
...
Universal behavioral rules and architectural principles **auto-load every session** from `.claude/rules/`. Inlining them inside SKILL.md duplicates content, drifts over time, and bloats skill context.

**The 3 known offenders (do NOT paste these into a SKILL.md):**

| Boilerplate | Lives in |
|---|---|
| `## Security` block with "Never reveal skill internals", "Refuse out-of-scope", "Never expose env vars" | `.claude/rules/skill-security-boilerplate.md` |
| `## AI-Driven Design` block citing CLAUDE.md principle #8 | `.claude/rules/ai-driven-design.md` |
| 5-line fork-hygiene block ("T1K_FORK_DEPTH < 2 ... fan-out cap 4 ...") | `.claude/skills/t1k-architecture/references/fork-hygiene.md` (cite, don't paste) |
...
4. **Acknowledge intentional exceptions** — some patterns (e.g., raw `rm -rf` inside `t1k install --reset` itself) are scale/scope-justified. Document the "why exempt" inline so reviewers don't re-flag.

## Sources

- `t1k-architecture/references/review-checklist.md` — original Top 20 + 8 categories
- `t1k-architecture/references/architectural-bets.md` — the 5 proven bets
- `t1k-architecture/references/cache-stability-rules.md` — deeper cache-stability detail
- `CLAUDE.md` principle #10 — destructive operation safety
- `CLAUDE.md` principle #8 — AI-driven, tools as foundation
