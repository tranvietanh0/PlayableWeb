...
- [ ] No live shell substitution in skill body (bang-prefixed backtick command form)
...
### G. Destructive operation safety (Category I — CLAUDE.md #10)
...
- [ ] Recovery instructions wrap destructive ops in CLI command (`t1k install --reset` style) that backs up first

### H. Universal rules hygiene
- [ ] Skill body does NOT inline universal rules. The 3 forbidden boilerplates (skill-security, AI-Driven Design, fork-hygiene 5-line) live in `.claude/rules/` or `references/fork-hygiene.md`. See `architecture-rules.md` → "Anti-Pattern: Inlining Universal Rules".
...
**Anti-pattern (weak):**
```
- Use `t1k-tester` agent for testing
```

**Correct pattern (enforceable):**
```
- **MUST** spawn `t1k-tester` subagent: `Task(subagent_type="t1k-tester", prompt="Run tests", description="Test")`
