...
- If description only: spawn t1k-planner teammate first:
  ```
  Agent(
    subagent_type: "{resolved t1k-planner agent}",
...
3. `TaskCreate` x 1 t1k-tester task:
...
After all merged:
  e. Run full test suite (spawn t1k-tester)
...
```
Agent(
  subagent_type: "{resolved t1k-tester agent}",
  name: "t1k-tester",
...
```
Agent(
  subagent_type: "t1k-fullstack-developer",
  name: "merger",
