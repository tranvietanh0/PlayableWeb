...
Use this when: invoked by `telemetry-kit-error-collector.cjs` via background sub-agent. The parent assistant reads a `[t1k:auto-issue]` marker from hook output and spawns a sub-agent with a pending submission entry from `.claude/telemetry/pending-issue-submissions.jsonl`.
...
```json
{
  ...
  "origin": {
    "kit": "theonekit-<name>",
    "kitVersion": "from .claude/metadata.json",
    "repository": "Owner/repo",
    "module": "string or null",
    "moduleVersion": "from module.json, or null"
  },
  "environment": {
    "cliVersion": "from t1k --version",
