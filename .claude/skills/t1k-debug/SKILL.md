---
name: t1k:debug
description: "Investigate and diagnose errors via registry-routed t1k-debugger. Root cause first, no fixes until confirmed. Use for runtime errors, test failures, CI/CD issues, log analysis."
...
| Stuck after 3+ hypothesis-test cycles | Switch to `/t1k:problem-solve` (assumption-break) |
...
This command uses role: `t1k-debugger`
...
**Load when:** Multi-component investigation (3+ steps), parallel log collection, coordinating t1k-debugger subagents
...
Before spawning t1k-debugger agent, inject module context.
