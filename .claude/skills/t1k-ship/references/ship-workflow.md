...
2. Delegate to `t1k-tester` subagent — don't inline test execution
...
2. Delegate to `t1k-code-reviewer` subagent with the diff
3. Two-pass model:
...
1. Invoke `/t1k:docs update` via `t1k-docs-manager` subagent in background:
