---
name: t1k:negotiation
description: "Comprehensive negotiation playbook — counterparty pattern recognition, BATNA/ZOPA prep, multi-round phase guidance, cultural code-switching, named techniques (80+) with Tell/Use/Counter framing. Use BEFORE any commercial negotiation (term sheets, partnership agreements, M&A, distribution, licensing, advisor equity, vendor contracts, salary, partnership disputes). Modes: (1) `analyze` — read counterparty markup/email/transcript and classify pattern + suggest counter; (2) `prep` — pre-negotiation BATNA + leverage map + cultural brief for a specific counterparty; (3) `playbook` — phase-aware guidance (pre/opening/middle/closing/post-signature) for current round; (4) `culture` — cultural code-switcher: tone, timing, face-saving, agreement-signal mapping for one counterparty's culture. Anchored to canonical literature (Fisher/Ury, Voss, Karrass, Shell, Bazerman, Meyer, Malhotra, Cialdini, Kahneman, Schelling, Galinsky, Noesner). Complements t1k:contract-review (clause craft) and t1k:contract (rendering)."
keywords: [negotiate, negotiation, negotiating, counter-offer, counter-proposal, BATNA, ZOPA, walk-away, term sheet feedback, counterparty pattern, cultural negotiation, cross-cultural deal, M&A deal, advisor equity, concession, anchor, anchoring, pre-mortem, bargain, dealmaking, influence, persuasion, closing the deal, deal making, partnership negotiation, vendor negotiation, salary negotiation, contract negotiation, redline counter, mirror, label, calibrated questions, tactical empathy, decreasing-step, ackerman, focal point, credible commitment, nemawashi, nunchi, guanxi, mianzi, kibun, jeong, honne, tatemae, kraljic matrix, columbo technique, log-rolling, package deal, conditional concession, splitting the difference, escalation of commitment]
argument-hint: "analyze <input.md|transcript> | prep <counterparty> | playbook <phase> | culture <country>"
effort: high
version: 1.94.2
...
- `t1k-contract-review` — **clause craft**: tilt analysis, redlining, balance review, Schedule A drafting, cover-email mechanics. Use AFTER negotiation strategy is set.
- `t1k-contract` — **rendering and beautification**: pandoc lint, CSS, docx/pdf generation. Use LAST.
...
3. Hand off to `t1k:contract-review` for clause-by-clause redlining
4. Use `playbook` Phase 3 (middle game) to plan your reply
5. Hand off to `t1k:contract` to render the response document
...
4. Hand off to `t1k:contract-review` mode `balance` to draft your opening posture
...
- **Don't run `analyze` and `t1k:contract-review` in parallel.** Strategy first, clause craft second. Running them together creates bad clause edits informed by half-formed strategy.
...
- **Pure clause craft** (redlining, schedule drafting, render). Use `t1k:contract-review` for clauses, `t1k:contract` for render.
