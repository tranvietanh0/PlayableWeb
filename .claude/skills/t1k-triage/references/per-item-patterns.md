...
| `needs-fix` | Content has a real issue (CI fail, missing field, conflict, broken claim, bot-flagged critical security finding) — list findings | `decision=solve` (cook) by default — see "Fix-first policy" below |
| `defer` | Cannot decide without more context — list missing fields. **"Multiple candidate fixes documented in the body" is NOT a `defer` cause under `--yolo`** — see yolo-decision-matrix.md row "defer because issue requires a UX/policy decision". Under `--yolo`, defer is reserved for: missing repro, calendar gates, upstream prerequisite missing, external blocker | `decision=defer` |

### Fix-first policy (default for `needs-fix`)

**Default action for `needs-fix` is `solve` (fix-and-merge), not `defer`.** This applies regardless of whether the issue was raised by a human reviewer or a bot review (`the1studio-code-reviewer` COMMENTED state with critical findings counts the same as `CHANGES_REQUESTED`).

Switch from `solve` to `leave-open-for-author` ONLY when ALL of the following hold:

1. **External fork** — `headRepositoryOwner.login` does NOT match the base-repo owner (no write access; comment-with-patch only per `yolo-merit-pipeline.md` line 28-30).
2. **Author is actively responding** — last author commit OR PR comment within the past 24h.
3. **Findings require domain knowledge the author owns** — e.g., business-rule correctness, not generic security hardening.

Same-org branches (even from another developer) → default to `solve`. The maintainer's commits land on top of the author's; admin-merge cites "admin-override (yolo) — fix pushed in {sha}". The author is informed via the standard PR-update notification; they can request a revert if they disagree.

**Anti-pattern (2026-05-15 cocos#23 misclassification):** four critical security findings (SSRF / OOM / redirect / prompt-injection) on a same-org PR were tagged `defer: needs-fix` because the bot review state was `COMMENTED` rather than `CHANGES_REQUESTED`. The correct verdict was `needs-fix` → `solve`: maintainer pushes the security fixes themselves, then admin-merges. Defer is only correct when fix cannot be authored without missing context.
