...
| Skill-file gate | If PR touches `$HOME/.claude/skills/`, must have run `/t1k:skill-creator validate <skill-name>` (Step 4) | Skillmark conventions verified |
...
1. **Step 4** — Spawn `t1k-code-reviewer` agent. Returns `approve` / `request-changes` / `comment`
...
- t1k-code-reviewer verdict = `approve`
- Step 4b risk = `low`
...
- t1k-code-reviewer = `request-changes` → post review via `gh pr review --request-changes`; mark `merge-blocked: code-reviewer-rejected`
- t1k-code-reviewer = `comment` → post review as comment; mark `merge-blocked: code-reviewer-uncertain`
