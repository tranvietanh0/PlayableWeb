...
- Per-item failure → log `T1K_DEBUG_TRIAGE` entry, skip comments for that item, continue triage (do NOT abort full run)
...
**Credential sanitization** — if an issue/PR body or comment contains what looks like a credential (API key, bearer token, JWT, AWS key id, private key fragment), redact it before quoting in the triage report or any downstream artifact. The `t1k-issue` template's `SENSITIVE_PATTERNS` regex set is the reference; mirror it here.

**Debug:** set `T1K_DEBUG_TRIAGE=1` to log per-item enrichment fetches + classifier inputs to stderr.

## Step 1d — contribution score backfill (optional)

For each enriched issue/PR from Step 1c (cap at first **20 items per triage run** to bound AI cost), invoke `t1k:contribution-score` with `type=triage-backfill`, the item's `html_url` as `ref_url`, and the title + body already in classifier RAM. The contribution-score skill is the SSOT — do NOT inline rubric or POST logic here.

The worker idempotency on `(user, ref_url)` makes this safe: items already scored at filing time (via `t1k:issue` / `t1k:sync-back`) return `already_recorded` and are not double-counted. Backfill catches items that pre-date the AI scoring path or were filed manually on GitHub.
...
After Step 2c, evaluate **each issue** against the required-fields list. Do the same for sync-back PRs against `t1k-sync-back`'s required PR-body sections.

**Required fields for issues (per `t1k-issue` template):**
- `kit` + `kitVersion`
...
**Required fields for sync-back PRs (per `t1k-sync-back` PR format):**
...
3. `--auto` mode SKIPS these items entirely — they are NOT passed to `/t1k:cook`.
