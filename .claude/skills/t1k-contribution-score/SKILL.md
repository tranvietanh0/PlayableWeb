---
name: t1k:contribution-score
description: "Internal SSOT skill: AI-score a contribution artifact (issue/PR) and POST to telemetry worker. Invoked by t1k:issue, t1k:sync-back, t1k:triage — not user-facing. Use only when an existing skill needs to record a contribution score."
keywords: [contribution score, score artifact, record contribution]
argument-hint: "--type=issue|sync-back-pr|triage-backfill --ref-url=URL --title=... --body=..."
effort: low
version: 1.115.3
...
```bash
case "$REPO" in
  The1Studio/theonekit-*|The1Studio/t1k-*) ;;
...
2. `<project>/.claude/t1k-config-core.json` → `telemetry.cloud.endpoint`, then strip trailing `/ingest` if present
